...
- 未同期キュー（D）: オフライン時の口コミ・投票は `kirei-toilet-pending-*-v1` に
  保存され、起動時・`online` 復帰時に自動再送する。キュー処置の判定は
  `src/lib/localDeltas.ts` の `pendingQueueAction` に一本化:
  - **2xx**: サーバー応答（確定値）を状態へ反映してキューから外す。
    ただし本文が HTML の場合（PWAシェル・未知 `/api/*` パスの SPA フォールバックで、
    リクエストがAPIに届いていない可能性）は例外的にキューへ保持する
  - **4xx（400/404/409 などサーバー応答ありの確定拒否）**: キューから破棄して
    再送しない（重複投稿・スパムURL・モデレーション済みレビューへの投票など、
    再送しても結果が変わらないため。投票の楽観表示は次回のサーバー同期で修正される）
  - **429（レート制限）と 5xx・ネットワーク断（fetch自体の失敗）**: 時間経過で
    結果が変わる一時的な拒否のためキューに保持し、次回の起動時・`online` 復帰時に
    再送する（レート制限は制限ウィンドウ経過後に同じリクエストが成功しうる）
